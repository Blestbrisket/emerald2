...
u32 AbilityBattleEffects(u32 caseID, u32 battler, u32 ability, u32 special, u32 moveArg)
{
    ...
    case ABILITYEFFECT_MOVE_END: // Think contact abilities.
        ...
        case ABILITY_FLARE_UP:
            ...
            {
                gEffectBattler = battler;
                SET_STATCHANGER(STAT_SPATK, 3, FALSE);
                BattleScriptPushCursor();
                gBattlescriptCurrInstr = BattleScript_TargetAbilityStatRaiseRet;
                effect++;
            }
        ...
        case ABILITY_HYPNOTIST:
            if (!(gBattleStruct->moveResultFlags[gBattlerTarget] & MOVE_RESULT_NO_EFFECT)
             ...
             && GetBattlerHoldEffect(gBattlerAttacker, TRUE) != HOLD_EFFECT_PROTECTIVE_PADS
             && IsMoveMakingContact(move, gBattlerAttacker))
             && (B_ABILITY_TRIGGER_CHANCE >= GEN_4 ? RandomPercentage(RNG_HYPNOTIST, 20) : RandomChance(RNG_HYPNOTIST, 1, 3))
            {
                    if (IsSleepClauseEnabled())
                        gBattleStruct->battlerState[gBattlerAttacker].sleepClauseEffectExempt = TRUE;
                    gBattleScripting.moveEffect = MOVE_EFFECT_AFFECTS_USER | MOVE_EFFECT_SLEEP;
        ...
        case ABILITY_STEAM_ENGINE:
            ...
            break;
        case ABILITY_GEOTHERMAL:
            if (!(gBattleStruct->moveResultFlags[gBattlerTarget] & MOVE_RESULT_NO_EFFECT)
             && IsBattlerTurnDamaged(gBattlerTarget)
             && IsBattlerAlive(battler)
             && CompareStat(battler, STAT_SPATK, MAX_STAT_STAGE, CMP_LESS_THAN)
             && (moveType == TYPE_FIRE || moveType == TYPE_GROUND))
            {
                gEffectBattler = battler;
                SET_STATCHANGER(STAT_SPATK, 6, FALSE);
                BattleScriptPushCursor();
                gBattlescriptCurrInstr = BattleScript_TargetAbilityStatRaiseRet;
                effect++;
            }
        ...
        case ABILITY_SAND_SPIT:
            ...
            break;
        case ABILITY_SNOW_FLURRY:
            if (!(gBattleStruct->moveResultFlags[gBattlerTarget] & MOVE_RESULT_NO_EFFECT)
             && !gProtectStructs[gBattlerAttacker].confusionSelfDmg
             && IsBattlerTurnDamaged(gBattlerTarget)
             && !(gBattleWeather & B_WEATHER_SNOW && HasWeatherEffect()))
            {
                if (gBattleWeather & B_WEATHER_PRIMAL_ANY && HasWeatherEffect())
                {
                    BattleScriptPushCursor();
                ...
                else if (TryChangeBattleWeather(battler, BATTLE_WEATHER_SNOW, TRUE))
                {
                    gBattleScripting.battler = battler;
                    BattleScriptPushCursor();
                    gBattlescriptCurrInstr = BattleScript_SandSpitActivates;
        ...
        case ABILITY_BLIND_FURY:
            ...
            {
                BattleScriptPushCursor();
                gBattlescriptCurrInstr = BattleScript_BlindFuryActivates;
                effect++;
            }
            break;
    ...
    case ABILITYEFFECT_MOVE_END_ATTACKER: // Same as above, but for attacker
        ...
        case ABILITY_FIRE_FIGHTER:
            if (!(gBattleStruct->moveResultFlags[gBattlerTarget] & MOVE_RESULT_NO_EFFECT)
             ...
             && RandomPercentage(RNG_FIRE_FIGHTER, 30))
            {
...
u32 IsAbilityPreventingEscape(u32 battler)
{
    ...
    if ((id = IsAbilityOnOpposingSide(battler, ABILITY_MAGNET_PULL)) && IS_BATTLER_OF_TYPE(battler, TYPE_STEEL))
        return id;
    if ((id = IsAbilityOnOpposingSide(battler, ABILITY_BUG_TRAP)) && IS_BATTLER_OF_TYPE(battler, TYPE_BUG))
        return id;
    if ((id = IsAbilityOnOpposingSide(battler, ABILITY_FISHERMON)) && IS_BATTLER_OF_TYPE(battler, TYPE_WATER))
        return id;
...
static inline u32 CalcAttackStat(struct DamageCalculationData *damageCalcData, u32 atkAbility, u32 defAbility, u32 holdEffectAtk, u32 weather)
{
    ...
    case ABILITY_GORILLA_TACTICS:
        if (IsBattleMovePhysical(move))
            modifier = uq4_12_multiply(modifier, UQ_4_12(1.5));
        break;
    case ABILITY_ROCK_DRUMMER:
    case ABILITY_ROCKY_PAYLOAD:
        if (moveType == TYPE_ROCK)
            modifier = uq4_12_multiply(modifier, UQ_4_12(1.5));
        break;
    case ABILITY_EXCAVATOR:
        if (moveType == TYPE_GROUND)
            modifier = uq4_12_multiply(modifier, UQ_4_12(1.5));
        break;    
    case ABILITY_PROTOSYNTHESIS:
...
static inline uq4_12_t GetDefenderAbilitiesModifier(u32 move, u32 moveType, u32 battlerAtk, u32 battlerDef, uq4_12_t typeEffectivenessModifier, u32 abilityDef)
{
    ...
    case ABILITY_FLUFFY:
        ...
        if (IsMoveMakingContact(move, battlerAtk) && moveType != TYPE_FIRE)
            return UQ_4_12(0.5);
        break;
    case ABILITY_GASEOUS:
        if (!IsMoveMakingContact(move, battlerAtk) && moveType == TYPE_FIRE)
            return UQ_4_12(2.0);
        if (!IsMoveMakingContact(move, battlerAtk) && moveType == TYPE_FLYING)
            return UQ_4_12(1.5);
        if (IsMoveMakingContact(move, battlerAtk) && moveType != TYPE_FIRE && moveType != TYPE_FLYING)
            return UQ_4_12(0.75);
        break;
    case ABILITY_PUNK_ROCK:
        if (IsSoundMove(move))
